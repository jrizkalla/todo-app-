...
enum TodoQueries {
    ...
    static func inbox(_ todos: [Todo]) -> [Todo] {
        topLevel(todos)
            .filter { $0.bucket == .inbox && !$0.state.isResolved && !$0.isProject }
    ...
    /// date.
    static func today(_ todos: [Todo], calendar: Calendar = .current, now: Date = Date()) -> [Todo] {
        let endOfToday = calendar.startOfDay(for: now).addingTimeInterval(24 * 3600)

        return topLevel(todos)
            .filter { todo in
                guard !todo.state.isResolved else { return false }
    ...
    static func thisWeek(_ todos: [Todo], calendar: Calendar = .current, now: Date = Date()) -> [Todo] {
    ...
    static func anytime(_ todos: [Todo]) -> [Todo] {
        topLevel(todos)
            .filter { $0.bucket == .anytime && !$0.state.isResolved && !$0.isProject }
            .sorted(by: sortByDateThenOrder)
    }
    ...
    static func inSpace(_ todos: [Todo], spaceID: UUID) -> [Todo] {
        todos
            .filter { $0.space?.uuid == spaceID && $0.parent == nil && !$0.isProject && !$0.state.isResolved }
            .sorted(by: sortByOrder)
    }

    /// Direct children of a project.
    static func inProject(_ todos: [Todo], projectID: UUID) -> [Todo] {
        todos
            .filter { $0.parent?.uuid == projectID && !$0.state.isResolved }
            .sorted(by: sortByOrder)
    }
    ...
    static func scheduled(_ todos: [Todo], on day: Date, calendar: Calendar = .current) -> [Todo] {
        ...
        return topLevel(todos).filter { todo in
            guard !todo.state.isResolved, let assigned = todo.assignedDate else { return false }
